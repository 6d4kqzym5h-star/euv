...
#[test]
fn toggle_new_with_true() {
    let toggle: Toggle = Toggle::new(true);
    assert!(toggle.get());
}

#[test]
fn toggle_new_with_false() {
    let toggle: Toggle = Toggle::new(false);
    assert!(!toggle.get());
}
...
fn toggle_clone_shares_state() {
    let original: Toggle = Toggle::new(false);
...
fn toggle_partial_eq_same_value() {
    let a: Toggle = Toggle::new(true);
    let b: Toggle = Toggle::new(true);
    assert_eq!(a, b);
}
