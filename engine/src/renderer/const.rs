// Allow dead-code warnings: this file documents the *entire* WebGPU
// descriptor surface (formats, usage flags, method names, property keys).
// Individual rounds of engine work only consume a subset; the rest are
// reserved for the public API the user can call from euv, so we
// intentionally do not delete unused constants.
#![allow(dead_code)]
