...
pub(crate) fn extract_touch_points(event: &Event) -> Vec<NativeTouchPoint> {
...
/// from a `TouchEvent`.
///
/// Similar to `extract_touch_points`, but returns `f64` precision for
...
pub(crate) fn extract_touch_points_f64(event: &Event) -> Vec<NativeTouchPointF64> {
    let touches_value: JsValue = Reflect::get(event.as_ref(), &JsValue::from_str("touches"))
        .ok()
        .unwrap_or(JsValue::NULL);
    let touches: Array = touches_value.unchecked_into();
...
pub(crate) fn extract_changed_touch_points(event: &Event) -> Vec<NativeTouchPoint> {
    let touches_value: JsValue = Reflect::get(event.as_ref(), &JsValue::from_str("changedTouches"))
        .ok()
        .unwrap_or(JsValue::NULL);
    let touches: Array = touches_value.unchecked_into();
    let target: JsValue = event
