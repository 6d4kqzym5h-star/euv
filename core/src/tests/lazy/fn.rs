...
fn change_factory_resets_state() {
    // Signal::set panics on native targets because
    // js-sys imported statics are wasm-only. The
    // reset behavior is observable on wasm.
    ...
    {
        // Skip on native: get() panics inside
        // signal.set due to js-sys.
