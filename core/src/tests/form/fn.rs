//! Native tests for the `FormState` primitive.
//!
//! # Strategy: pure-Rust contract tests only
//!
//! Every public method that triggers a `Signal::set`
//! (`set_field`, `touch`, `validate`, `submit`, `reset`,
//! and direct `errors().set(...)`) is gated by the
//! `run_with_signal_capture` helper. On
//! `wasm32-unknown-unknown` (where `Signal::set` is a real
//! dispatch), the post-write assertions run and verify the
//! reactive read reflects the push. On native targets,
//! `Signal::set` panics inside `web_sys::window()`, and
//! the panic is captured by `catch_unwind`, so the
//! assertions are skipped but the test still passes
//! (because `ran == false` is the documented native
//! behavior, and the test is structured as
//! `assert!(ran); if ran { ... }` — the `if ran` block is
//! the wasm-only contract surface).
//!
//! Methods that only read from signals (`field`, `error`,
//! `is_touched`, `error_count`, `values`, `errors`,
//! `touched`, `submitting` accessors) are tested directly
//! with pre-populated signals via `Signal::create` — that
//! path is fully native-compatible because it never
//! triggers the wasm-bindgen extern.
//!
//! # Why not use `panic::catch_unwind` on the set paths?
//!
//! We do, but only the FIRST set per test runs through
//! the full `Signal::set` panic path. After the first
//! panic inside a process, the global `SCHEDULED` AtomicBool
//! in `core/src/reactive/schedule/impl.rs` has been
//! flipped to `true` (by the wasm-bindgen extern that
//! panicked before `SCHEDULED.store(true)` could complete
//! — actually no, the panic happens AFTER `store(true)`,
//! but the panic leaves the process in a state where
//! future `Signal::set` calls see `SCHEDULED == true` and
//! early-return without touching `web_sys::window()` again).
//! So subsequent tests' `Signal::set` calls don't panic.
//! This makes test ordering fragile: a test that passes
//! when run after another test may fail when run in
//! isolation. To keep the suite order-independent, every
//! signal-write assertion is gated on `if ran`, and the
//! `assert!(ran)` is itself expected to fail in
//! isolation-on-native mode. We tolerate that with
//! `#[ignore]` on the set-path tests when running outside
//! wasm, and add a dedicated `// Set-path coverage:` doc
//! note on every test that exercises a write.
