//! Native tests for the `ProfilerHandle` / `ProfileEntry` /
//! `now_ms` building blocks.
//!
//! The profiler is a pure-Rust reactive primitive (no DOM
//! access in the measurement path), so every public surface
//! is exercised here without spinning up a browser:
//!
//! - `now_ms()`: monotonic, non-decreasing, sane resolution
//!   on native targets.
//! - `ProfileEntry`: `new`, `Clone`, `PartialEq`, field
//!   access.
//! - `ProfilerHandle`: `new`, `Clone`, `begin`/`end`,
//!   `measure`, `entries` reactivity, `clear`.
//! - `ProfilerMark::end`: pushes the entry, captures
//!   `elapsed_ms >= 0.0`.
//!
//! Tests do NOT exercise `App::use_profiler()` (which is a
//! hook-context-bound factory); that path is covered by
//! `App` integration tests in `tests/app/`. Here we test the
//! handle as a plain `Data` struct, exactly the way a
//! caller who constructed it manually would.
//!
//! # Native-target caveat: `Signal::set` panics
//!
//! `ProfilerHandle::measure` and `ProfilerMark::end` push
//! into `entries` via `Signal::set`. On `wasm32-unknown-unknown`,
//! that path works as expected. On native targets
//! (`cargo test`), `Signal::set` delegates to
//! `App::schedule_update`, which calls `web_sys::window()` —
//! and `web_sys::window()` panics with "cannot access
//! imported statics on non-wasm targets" because wasm-bindgen
//! externs are stubbed-out panic functions on native.
//!
//! This is a known euv-core limitation (the existing
//! `tests/signal/fn.rs` test suite wraps every `Signal::set`
//! call in `panic::catch_unwind`). We do the same: every
//! profiler test that triggers a `Signal::set` wraps the
//! measurement side-effect in `catch_unwind` and verifies the
//! pre-push invariants instead. The closure-return /
//! type-shape contracts (which DO run entirely in user code,
//! before the signal write) are asserted directly. The
//! post-push invariants (`entries().get().len() == 1` etc.)
//! are verified only inside the `catch_unwind` — meaning on
//! wasm they run for real, on native they are skipped but
//! the panic is also captured.
//!
//! Helper: `run_with_signal_capture` runs the closure and
//! catches the native-build `Signal::set` panic, returning
//! `true` if the closure ran to completion (wasm path) and
//! `false` if it panicked (native path). Tests then branch
//! on the return value to gate their full-vs-partial
//! assertions.
