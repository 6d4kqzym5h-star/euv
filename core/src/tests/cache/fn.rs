...
fn get_returns_value_and_updates_recency() {
    ...
    // Now `a` is the most-recently-used; `b` is
...
fn put_evicts_least_recently_used_when_full() {
    ...
    cache.put("a".to_string(), 1);
    cache.put("b".to_string(), 2);
    // Cache is full; next put evicts `a`.
...
fn remove_drops_order_entry_too() {
    ...
    // After removing `a`, only `b` should be in
    // the order deque. Re-insert `a` should not
    // promote it incorrectly.
...
fn contains_does_not_update_recency() {
    ...
    cache.put("a".to_string(), 1);
    cache.put("b".to_string(), 2);
    // `contains` should NOT promote.
    assert!(cache.contains(&"a".to_string()));
    // Evicting LRU — should be `a` because `b` was
    // inserted last and `contains` did not change
    // recency.
    let evicted = cache.put("c".to_string(), 3);
    assert_eq!(evicted, Some(("a".to_string(), 1)));
}
...
fn resize_smaller_evicts_lru() {
    ...
    cache.resize(2);
    assert_eq!(cache.len(), 2);
    // LRU order: c, b, a. Resize to 2 keeps c, b.
    assert!(cache.contains(&"c".to_string()));
    assert!(cache.contains(&"b".to_string()));
    assert!(!cache.contains(&"a".to_string()));
}
...
fn peek_does_not_update_recency() {
    ...
    // `a` is still LRU; capacity-2, evicting on
    // next put.
    let evicted = cache.put("c".to_string(), 3);
    assert_eq!(evicted, Some(("a".to_string(), 1)));
}
...
fn integer_key_works() {
    ...
    // 1 was promoted by `get`, so 2 is evicted.
